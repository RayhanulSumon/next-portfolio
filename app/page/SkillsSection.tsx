...
function Skills3DBackground() {
	return (
		<div className="absolute inset-0 w-full h-full z-0 pointer-events-none" style={{ filter: 'blur(8px)', opacity: 0.25 }}>
			<Canvas camera={{ position: [0, 0, 4], fov: 50 }} style={{ width: '100%', height: '100%' }}>
				<ambientLight intensity={0.7} />
				<directionalLight position={[2, 2, 2]} intensity={0.5} />
				{/* Multiple animated spheres for a dynamic background */}
				<Sphere args={[0.7, 32, 32]} position={[-1.5, 0.5, 0]}>
					<MeshDistortMaterial color="#38bdf8" speed={2.5} distort={0.35} opacity={0.7} transparent />
				</Sphere>
				<Sphere args={[0.5, 32, 32]} position={[1.2, -0.7, 0]}>
					<MeshDistortMaterial color="#0ea5e9" speed={2.2} distort={0.25} opacity={0.7} transparent />
				</Sphere>
				<Sphere args={[0.4, 32, 32]} position={[0.5, 1.2, 0]}>
					<MeshDistortMaterial color="#6366f1" speed={2.8} distort={0.3} opacity={0.7} transparent />
				</Sphere>
			</Canvas>
		</div>
	);
			...
			<div className="container mx-auto px-4 flex flex-col items-center justify-center z-10">
				...
				>
					{skills.map((skill) => (
						<motion.div
							key={skill.name}
							className="flex flex-col items-center bg-white dark:bg-slate-900 rounded-2xl shadow-xl p-6 hover:scale-105 transition-transform backdrop-blur-md"
